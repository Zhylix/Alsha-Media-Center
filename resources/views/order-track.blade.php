...
<section class="relative py-24 bg-hero overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <h1 class="text-4xl font-black text-white mb-4">Lacak <span class="text-gradient">Pesanan</span></h1>
        <p class="text-slate-400">Masukkan nomor pesanan Anda untuk melihat status terkini.</p>
    </div>
</section>
<section class="py-16">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="service-card p-8 rounded-2xl">
            ...
            <div class="relative mb-8">
                ...
                <div class="flex justify-between relative">
                    ...
                    @foreach([
                        ...
                        ['key'=>'confirmed','label'=>'Dikonfirmasi','icon'=>'<i class="fas fa-check text-green-500"></i>'],
                        ['key'=>'in_progress','label'=>'Diproses','icon'=>'<i class="fas fa-wrench text-orange-400"></i>'],
                        ['key'=>'completed','label'=>'Selesai','icon'=>'<i class="fas fa-check-circle text-green-500"></i>'],
                    ] as $i => $s)
                    ...
                    <div class="flex flex-col items-center z-10 relative">
                        ...
                        <p class="text-xs mt-2 {{ $isCurrent ? 'text-blue-400 font-bold' : ($isDone ? 'text-slate-300' : 'text-slate-600') }}">{{ $s['label'] }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between"><span class="text-slate-400">Pelanggan</span><span class="text-white">{{ $order->customer_name }}</span></div>
                ...
                <div class="flex justify-between"><span class="text-slate-400">Status Pembayaran</span>
            ...
            <div class="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                <p class="text-yellow-400 text-xs font-bold mb-1">Catatan dari Admin:</p>
                <p class="text-slate-300 text-sm">{{ $order->notes }}</p>
            </div>
            @endif
        </div>
        ...
        <div class="service-card p-8 rounded-2xl text-center">
            ...
            <h3 class="font-bold text-white mb-2">Pesanan Tidak Ditemukan</h3>
            <p class="text-slate-400 text-sm">Nomor pesanan "{{ request('order_number') }}" tidak ditemukan. Pastikan nomor yang Anda masukkan benar.</p>
        </div>
        @endif
    </div>
</section>
