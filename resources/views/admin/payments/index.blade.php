...
<tr>
<td class="text-gray-900 font-medium text-sm">{{ $m->name }}</td>
<td><span class="badge badge-{{ $m->type==='bank_transfer'?'blue':($m->type==='e_wallet'?'purple':'green') }}">{{ $m->type_label }}</span></td>
<td class="text-gray-600 text-sm">{{ $m->provider }}</td>
<td class="text-gray-700 text-sm font-mono">{{ $m->account_number ?? '-' }}</td>
<td><span class="badge badge-{{ $m->is_active?'green':'gray' }}">{{ $m->is_active?'Aktif':'Nonaktif' }}</span></td>
<td class="flex gap-2">
<a href="{{ route('admin.payments.edit', $m) }}" class="px-3 py-1.5 rounded-lg bg-red-600/10 text-red-600 text-xs">Edit</a>
